...
open is_group_hom mul_action

lemma mem_fixed_points_mul_left_cosets_iff_mem_normalizer {H : subgroup G} [fintype H]
  {x : G} : (x : quotient H) ∈ fixed_points H (quotient H) ↔ x ∈ H.normalizer :=
⟨λ hx, have ha : ∀ {y : quotient H}, y ∈ orbit H (x : quotient H) → y = x,
  from λ _, ((mem_fixed_points' _).1 hx _),
  H.normalizer.inv_mem_iff.1 (mem_normalizer_fintype (λ n hn,
    have (n⁻¹ * x)⁻¹ * x ∈ H := quotient_group.eq.1 (ha (mem_orbit _ ⟨n⁻¹, H.inv_mem hn⟩)),
    by {simp only [mul_inv_rev] at this, convert this; simp })),
...
(mem_fixed_points' _).2 $ λ y, quotient.induction_on' y $ λ y hy, quotient_group.eq.2
  ...
  H.inv_mem_iff.1 $ (hx _).2 $ (H.mul_mem_cancel_left (H.inv_mem hb₁)).1
  $ by rw hx at hb₂;
    simpa [mul_inv_rev, mul_assoc] using hb₂)⟩

def fixed_points_mul_left_cosets_equiv_quotient (H : subgroup G) [fintype H] :
  fixed_points H (quotient H) ≃ quotient (H.comap H.normalizer.subtype) :=
@subtype_quotient_equiv_quotient_subtype G H.normalizer.carrier (id _) (id _) (fixed_points _ _)
...
| 0 := λ _ _, ⟨⊥, by { convert card_trivial }⟩
| (n+1) := λ hp hdvd,
let ⟨H, hH⟩ := @exists_subgroup_card_pow_prime _ hp
...
have hcard : card (quotient H) = s * p :=
  (nat.mul_left_inj (show card H > 0, from fintype.card_pos_iff.2
      ⟨⟨1, H.one_mem⟩⟩)).1
    (by rwa [← card_eq_card_quotient_mul_card_subgroup, hH, hs,
      nat.pow_succ, mul_assoc, mul_comm p]),
    have hm : s * p % p = card (quotient (H.comap H.normalizer.subtype)) % p :=
  card_congr (fixed_points_mul_left_cosets_equiv_quotient H) ▸ hcard ▸
    @card_modeq_card_fixed_points _ _ _ _ _ _ _ p _ hp hH,
    have hm' : p ∣ card (quotient (H.comap H.normalizer.subtype)) :=
...
have fintype (mk ⁻¹' (gpowers x : set (quotient (H.comap H.normalizer.subtype)))), by apply_instance,
have hequiv : H ≃ H.comap H.normalizer.subtype :=
  ⟨λ a, ⟨⟨a.1, λ x, subgroup.le_normalizer a.2 _⟩, a.2⟩, λ a, ⟨a.1.1, a.2⟩,
    λ ⟨_, _⟩, rfl, λ ⟨⟨_, _⟩, _⟩, rfl⟩,
⟨((gpowers x).comap (quotient_group.mk_hom _)).map H.normalizer.subtype,
  by { simp only [subgroup.card_eq, subgroup.coe_map, subgroup.coe_comap, coe_fun_mk_hom],
    rw [set.card_image_of_injective (mk ⁻¹' ↑(gpowers x)) (subgroup.subtype.injective _),
      nat.pow_succ, ← hH, fintype.card_congr hequiv, ← hx, order_eq_card_gpowers,
      ← fintype.card_prod],
    apply fintype.card_congr (preimage_mk_equiv_subgroup_times_set _ _),
    exact this }⟩
